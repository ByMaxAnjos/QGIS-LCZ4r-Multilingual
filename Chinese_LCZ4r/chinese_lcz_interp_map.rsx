...
##QgsProcessingParameterRasterDestination|Output|保存地图
...
#'      :格式注意: 日期时间格式必须符合R惯例，如<b style='text-decoration: underline;'>2023-03-13 11:00:00</b>或<b style='text-decoration: underline;'>2023-03-13</b>。也可接受"1/2/1999"或"YYYY-mm-dd"、"1999-02-01"等格式。</p><p>
...
#' Date_start: 指定分析开始日期，格式为<b>DD-MM-YYYY [01-09-1986]</b>。
...
#'      :2. <b>两步</b>: 在分配LCZ时过滤位于异质LCZ区域的站点。要求5×5核内至少80%像元与中心像元LCZ匹配(Daniel等,2017)。会减少站点数量。常用于超高密度观测网络。</p><p>
...
#' Viogram_model: 如选择克里金，将测试的变异函数模型列表。默认为"Sph"。模型包括"Sph"(球面)、"Exp"(指数)、"Gau"(高斯)、"Ste"(Matern族，Stein参数化)。
#' Impute_missing_values: 填补缺失值的方法("平均值"、"中位数"、"knn"、"bag")。
#' LCZ_interpolation: 如为TRUE(默认)，使用LCZ插值方法。如为FALSE，使用常规克里金插值(不含LCZ)。
#' Output: terra格式的GeoTIF栅格。
#' ALG_DESC: 本函数利用LCZ和克里金法生成气温(或其他变量)的空间插值。</p><p>
#'         :更多信息请访问: <a href='https://bymaxanjos.github.io/LCZ4r/articles/local_func_modeling.html'>LCZ局部函数(基于LCZ的温度建模)</a>。
